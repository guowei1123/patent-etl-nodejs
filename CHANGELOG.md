...
- 新增 IPC/CPC 分类字典数据表、导入脚本和分类字典查询页，支持树状与列表视图检索
- 批次处理状态新增 XML 解析与附图上传进度展示，并在仪表盘显示 OSS 配置状态

### 变更

- 优化专利附图处理：上传前检查 OSS 已存在对象，支持上传超时、并发控制和跳过已存在附图统计
- 检索式中的 IPC/CPC 分类号查询改为标准化前缀匹配，兼容带空格分类号和 WIPO IPC 长码格式
...
- 批次详情页根据自动/手动校验日志展示下载文件和解压文件校验结果，已通过时隐藏对应的重复校验按钮

### 修复

- 解压文件 CRC 校验增加 CRC 清单与实际内层 ZIP 文件集合一致性检查，识别空 CRC 清单和未被 CRC 覆盖的 ZIP 文件
...
- 专利列表新增检索式查询，支持字段限定、AND/OR/NOT、括号、短语和公开日范围检索

### 修复

- 检索式语法错误在输入框下方展示，避免误作为普通数据加载失败处理
- 不支持的检索字段和非法公开日会在解析阶段返回明确错误，避免落到数据库层报错

## 0.6.6 - 2026-06-08

### 修复

- 删除批次时先删除数据库记录，再清理本地临时文件，避免数据库删除失败后本地文件被提前移除
- 本地临时目录清理限制在配置的临时根目录内，防止路径穿越误删其它目录
- 分卷 ZIP 合并、补丁或解压失败时清理已生成的 `.merged.zip` 临时文件
- 处理步骤在外层解压后自动校验内层 ZIP 的 CRC 和结构完整性，校验失败时停止解析
- Vitest 排除 `.next` 构建产物，避免生产构建后单元测试误执行 standalone 中复制的测试文件
...
- 导入失败日志新增结构化失败专利明细，记录专利号、kind、标题、来源文件和数据库错误信息
- 批次详情页展示导入失败专利列表，便于定位单条异常数据

### 修复

- 导入批量写入失败后二分定位单条异常数据时，不再静默计为 0；导入步骤会将部分成功、部分失败的结果标记为失败并保留已导入数量
- 补充导入失败明细和递归导入结果的单元测试
