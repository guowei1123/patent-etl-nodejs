...
从 https://lake.patentfun.cn/api/classifications 下载 IPC 分类号数据，
保存到本地 Redis Hash 中。

用法:
    python scripts/sync-ipc-to-redis.py [--max-pages N] [--batch-size N] [--concurrency N]
...
async def fetch_page(
    ...
    params = {"type": "ipc", "page": str(page), "limit": str(limit)}
    headers = {
...
async def sync_ipc_to_redis(
    cookie: str,
    max_pages: Optional[int] = None,
    batch_size: int = 20,
    redis_key: str = "classifications:ipc",
    concurrency: int = 50,
):
    ...
    async with aiohttp.ClientSession(
        ...
        async def worker(page: int):
            nonlocal total_synced, completed
...
def main():
    parser = argparse.ArgumentParser(description="同步 IPC 分类号数据到 Redis（异步并发版）")
    ...
    parser.add_argument("--redis-key", type=str, default="classifications:ipc", help="Redis Hash 键名")
